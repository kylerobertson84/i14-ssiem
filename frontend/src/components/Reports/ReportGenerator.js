...
const ReportGenerator = ({ onGenerate }) => {
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      ...
      setActiveStep(0);
      setIsRefreshing(false);
    }, 1000); // Simulating a refresh delay
  };
  ...
  return (
    <Paper elevation={3} sx={{ p: 4, borderRadius: 2, position: 'relative' }}>
      <Typography variant="h5" component="h2" gutterBottom sx={{ mb: 4, fontWeight: 'bold', color: 'primary.main', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        Generate New Report
        <Box>
          <Tooltip title="Refresh form">
            <IconButton onClick={handleRefresh} disabled={isRefreshing || isSubmitting}>
              <RefreshIcon />
            </IconButton>
          </Tooltip>
          ...
          <Box>
            {activeStep === steps.length - 1 ? (
              <Button
                ...
                disabled={!isStepComplete(activeStep) || isSubmitting || isRefreshing}
                sx={{
                  px: 4,
                  py: 1,
                  borderRadius: 2,
