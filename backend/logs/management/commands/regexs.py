...
MS_WMI_ACTIVITY = re.compile(
    ...
    r'SourceModuleType="(?P<source_module_type>[^"]+)"\]\s+'   
    r'Id\s*=\s*(?P<id>{[^}]+});\s*'
    r'ClientMachine\s*=\s*(?P<client_machine>[^;]+);\s*'
    r'User\s*=\s*(?P<user>[^;]+);\s*'
    r'ClientProcessId\s*=\s*(?P<client_process_id>\d+);\s*'
    r'Component\s*=\s*(?P<component>[^;]+);\s*'
    r'Operation\s*=\s*(?P<operation>[^;]+);\s*'
    r'ResultCode\s*=\s*(?P<result_code>[^;]+);\s*'
    r'PossibleCause\s*=\s*(?P<possible_cause>[^;]+)'
)
